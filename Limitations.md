...
## Migration scope (Global vs Company)

Use `-MigrationScope Global` once per source→target pair, then `-MigrationScope Company` per client (or all companies).

| Scope | Runs | Skips |
| ----- | ---- | ----- |
| **Global** | Asset layouts, lists, layout admin folders, central KB folders/articles, relink, flag types, tenant-wide passwords | Companies, company KB, procedures, websites, IPAM, photos, assets, racks, relations |
| **Company** | Companies, company KB, passwords, procedures, websites, IPAM, photos, assets, racks, relations, flags | Asset layouts, central KB (no longer bundled into SINGLE company mode) |
| **All** | Everything in one run (legacy behaviour) | — |

**Company** scope does not import central KB articles. Run **Global** first if the target needs layouts and central KB. Asset migration in **Company** scope expects layouts to already exist on the target (matched by name).
...
| **ListSelect** | Lists are migrated **before** asset layouts. Each source list is loaded with `GET /api/v1/lists/{id}` so `list_items` are populated (the index endpoint often returns empty `items`). Target lists are created with `list_items_attributes` via JSON API. Source list ids are mapped by **list name** (`Get-MigrationListMap`). ListSelect fields are **omitted** from the layout if the list could not be mapped. Asset field values are remapped to the **target list’s allowed options** (exact match, aliases, or `Other`). If a target list matched by name has no options but the source does, a warning is logged. |
